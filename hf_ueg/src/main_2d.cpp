...
int main() {
    ...
    Basis_2d pw_2d(ke_cutoff, rs, nelec);
    int n_pw = pw_2d.n_plane_waves();
    cout << "The number of plane waves within the kinetic energy cutoff is: " << n_pw << endl;
    assert(n_pw > 0 && n_pw < 250);

    arma::mat kinetic_integral_matrix = pw_2d.kinetic_integrals();
    arma::mat coulomb_integral_matrix = pw_2d.coulombIntegrals();

    // Generate initial guess for the density matrix
    arma::mat guess = pw_2d.generate_initial_guess();
